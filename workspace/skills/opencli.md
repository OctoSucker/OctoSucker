...
## AI Agent：`run_command` 调 opencli（必读）

OpenCLI 的进程 argv 形如：`opencli <一级子命令> <二级子命令> …`，**不能把整条 `https://x.com/...` URL 当作 `opencli` 的第一个参数**。若 `program` 为 `opencli` 且 `args` 只有 `[URL]`，等价于在 shell 里执行 `opencli https://...`，CLI 会把 URL 误当成**顶级命令名**，从而报错 `unknown command 'https://...'`。**这不是 skill 未加载，而是 argv 拼错。**

**阅读单条 X（Twitter）帖子（正文 + 回复 thread）**
...
```json
{ "program": "opencli", "args": ["twitter", "thread", "2040280672758345783", "-f", "md"] }
```

**推文长文**（与单条 status 不同 id 时）见下文 `opencli twitter article <id>`。需要 URL 作为参数时，仅适用于文档中写明 **url positional** 的子命令（如 `twitter like <url>`），**阅读 thread 请用 status 数字 id + `twitter thread`**。
