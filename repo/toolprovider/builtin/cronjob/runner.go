...
func (r *Runner) Name() (string, string) {
	return "cronjob", "Create, list, and delete scheduled jobs in the workspace."
}
...
func (r *Runner) Invoke(ctx context.Context, localTool string, arguments map[string]any) (types.ToolResult, error) {
	...
	default:
		return types.ToolResult{Err: fmt.Errorf("cronjob builtin: unknown tool %q", localTool)}, fmt.Errorf("cronjob builtin: unknown tool %q", localTool)
	}
}
