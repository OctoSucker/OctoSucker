...
func (r *Runner) Name() (string, string) {
	return "catalog", "LLM-only replies (greetings, Q&A) when no other tools are needed."
}
...
func (r *Runner) Invoke(ctx context.Context, localTool string, arguments map[string]any) (types.ToolResult, error) {
	...
	case ToolNaturalLanguageReply:
		...
		if err != nil {
			return types.ToolResult{Err: err}, nil
		}
		reply, err := r.llm.Complete(ctx, justChatSystemPrompt, um)
		if err != nil {
			return types.ToolResult{Err: fmt.Errorf("catalog builtin: %s: %w", ToolNaturalLanguageReply, err)}, fmt.Errorf("catalog builtin: %s: %w", ToolNaturalLanguageReply, err)
		}
		return types.ToolResult{Output: strings.TrimSpace(reply)}, nil
	default:
		return types.ToolResult{Err: fmt.Errorf("catalog builtin: unknown tool %q", localTool)}, fmt.Errorf("catalog builtin: unknown tool %q", localTool)
	}
}
