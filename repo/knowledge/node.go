...
// EnsureNode creates the node if it does not exist; if it already exists, it returns nil.
func (g *Graph) EnsureNode(ctx context.Context, id string) error {
	if id == "" {
		return fmt.Errorf("knowledgegraph: EnsureNode: empty id")
	}
	if g.db == nil {
		return fmt.Errorf("knowledgegraph: EnsureNode: graph has no db")
	}
	exists, err := g.db.KnowledgeGraphNodeExists(id)
	if err != nil {
		return fmt.Errorf("knowledgegraph: EnsureNode: db: %w", err)
	}
	if exists {
		return nil
	}
