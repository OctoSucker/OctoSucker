...
type ProcedureEntry struct {
	...
	// --- Macro procedure: route node sequence (capability::tool); Capabilities is the capability column per step (derived from Path) ---
	Capabilities []string
	Path         []string // routing-graph vertices: capability::tool
...
func (e ProcedureEntry) PreferredNodes() []string {
	if len(e.Path) == 0 {
		return nil
	}
...
// ProcedureLearnCapKeyFromTask returns a stable key for the route-node sequence (same as learned_* name suffix)
// and the number of plan steps. ok is false if there is no usable node path.
func ProcedureLearnCapKeyFromTask(taskState *ports.Task) (capKey string, planStepCount int, ok bool) {
	...
	nodes, err := planRouteNodes(taskState.Plan)
	if err != nil || len(nodes) == 0 {
		return "", planStepCount, false
	}
	return rtutils.HashPipeJoinedRouteNodes(nodes), planStepCount, true
}
...
func BuildProcedureEntryFromTask(ctx context.Context, taskState *ports.Task, embedder *llmclient.OpenAI) (ProcedureEntry, error) {
	...
	nodes, err := planRouteNodes(taskState.Plan)
	if err != nil {
		return ProcedureEntry{}, err
	}
	caps, err := deriveCapabilitiesFromPath(nodes)
	if err != nil {
		return ProcedureEntry{}, err
	}
	...
	return ProcedureEntry{
		Name:             "learned_" + rtutils.HashPipeJoinedRouteNodes(nodes),
		Capabilities:     caps,
		Path:             append([]string(nil), nodes...),
...
func planRouteNodes(p *ports.Plan) ([]string, error) {
	if p == nil || len(p.Steps) == 0 {
		return nil, ErrNoProcedureFromTask
	}
	out := make([]string, 0, len(p.Steps))
	for _, st := range p.Steps {
		if st.Capability == "" || st.Tool == "" {
			return nil, fmt.Errorf("procedure: plan step %q missing capability or tool for route node", st.ID)
		}
		n := routinggraph.NodeID(st.Capability, st.Tool)
		if n == "" {
			return nil, fmt.Errorf("procedure: plan step %q invalid capability/tool", st.ID)
		}
		out = append(out, n)
	}
	return out, nil
...
func deriveCapabilitiesFromPath(path []string) ([]string, error) {
	out := make([]string, 0, len(path))
	for _, node := range path {
		capID, _, ok := routinggraph.ParseNodeID(node)
		if !ok {
			return nil, fmt.Errorf("procedure: invalid route node %q", node)
		}
