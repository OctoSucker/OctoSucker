...
func (r *ProcedureRegistry) loadProceduresFromDB() error {
	...
	for rows.Next() {
		...
		for _, node := range path {
			if _, _, ok := routinggraph.ParseNodeID(node); !ok {
				return fmt.Errorf("procedure %s: path_json: invalid route node %q (want capability::tool)", name, node)
			}
		}
		capsFromPath, err := deriveCapabilitiesFromPath(path)
		if err != nil {
			return fmt.Errorf("procedure %s: %w", name, err)
		}
		if !rtutils.StringSlicesEqual(caps, capsFromPath) {
			return fmt.Errorf("procedure %s: caps_json does not match path_json", name)
		}
		e := &ProcedureEntry{
			Name:             name,
			Keywords:         kw,
