...
func (c *TrajectoryCritic) HandleTrajectoryCheck(ctx context.Context, evt ports.Event) (*ports.Event, error) {
	...
	if err := c.Tasks.Put(sess); err != nil {
		return nil, err
	}
	if sess.ReplanAllowed && sess.ReplanCount < maxReplanRounds && score < replanScoreThreshold && rtutils.TraceHasFailure(sess.Trace) {
		sess.ReplanCount++
		sess.UserInput.Text = sess.UserInput.Text + fmt.Sprintf("\n\n[系统：上轮轨迹评分 %.2f，存在失败步骤。请生成更短、更保守的计划。]", float64(score))
		if err := c.Tasks.Put(sess); err != nil {
			return nil, err
		}
		return ports.EventPtr(ports.Event{Type: ports.EvUserInput, Payload: ports.PayloadUserInput{TaskID: pl.TaskID, Text: sess.UserInput.Text, AutoReplan: true}}), nil
	}
