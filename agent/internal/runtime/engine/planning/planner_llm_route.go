...
func (p *Planner) HandleLLMPlanRequested(ctx context.Context, evt ports.Event) (*ports.Event, error) {
	...
	chunks, err := p.RecallCorpus.Recall(ctx, sess.UserInput.Text, 5)
	if err != nil {
		log.Printf("engine.Dispatcher: recall failed session=%s err=%v", pl.TaskID, err)
		return nil, fmt.Errorf("planner: recall: %w", err)
	}
	if len(chunks) > 0 {
		recallText = strings.Join(chunks, "\n---\n")
	}
	user := sess.UserInput.Text
	if recallText != "" {
		user = "相关记忆：\n" + recallText + "\n\n用户请求：\n" + sess.UserInput.Text
	}
	if sess.UserInput.TelegramChatID != 0 {
		user += fmt.Sprintf("\n\n[Channel: Telegram; current chat_id is %d. Include it as \"chat_id\" in step arguments when a tool requires chat_id.]", sess.UserInput.TelegramChatID)
	}
	sess.SkillPriorCaps = p.Skills.Match(sess.UserInput.Text)

	if len(p.ValidPlanCapabilities) == 0 {
		log.Printf("engine.Dispatcher: invalid plan JSON session=%s", pl.TaskID)
	...
	return p.finalizePlan(pl.TaskID, sess, parsed)
}
