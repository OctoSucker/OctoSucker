...
func NewRecallCorpus(embedder *llmclient.OpenAI, store *model.AgentDB) (*RecallCorpus, error) {
	m := &RecallCorpus{
		texts:      make([]string, 0),
...
func (m *RecallCorpus) loadAll() error {
	rows, err := m.store.RecallSelectAllOrdered()
	if err != nil {
		return err
	}
	m.mu.Lock()
	...
	for _, r := range rows {
		m.texts = append(m.texts, r.Text)
		m.embeddings = append(m.embeddings, rtutils.BlobToFloat32Slice(r.Embedding))
	}
	return nil
}
...
func (m *RecallCorpus) persistInsert(text string, vec []float32) error {
	...
	return m.store.RecallInsert(text, blob)
}
