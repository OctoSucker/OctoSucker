...
func (r *Runner) Tool(tool string) (*mcp.Tool, error) {
	...
	} else if r.cfg.backend == config.ExecBackendHost {
		desc = "Run a command directly on the host: set arguments.program to argv0; work_dir is still restricted to configured workspace roots"
	}
	return &mcp.Tool{
		Name:        ToolName,
		Description: desc,
...
func (r *Runner) runCommand(ctx context.Context, args map[string]any) (types.ToolResult, error) {
	...
	case config.ExecBackendHost:
		return r.runHost(runCtx, wd, program, argv, args)
	default:
