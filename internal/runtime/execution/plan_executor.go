...
// PlanExecutor advances the plan by running the next runnable tool synchronously and emitting EvStepObserved.
...
func (x *PlanExecutor) HandlePlanProgressed(ctx context.Context, pl types.StepScheduled) (*types.Event, error) {
	...
	if assessor, ok := x.ToolRegistry.(ToolPolicyAssessor); ok {
		policy := assessor.Assess(step.Node.Tool, argMap)
		task.AppendTrace("policy tool=%s risk=%s capabilities=%v summary=%s", step.Node.Tool, policy.Risk, policy.Capabilities, policy.Summary)
		if err := x.Tasks.Put(task); err != nil {
			return nil, err
		}
	}
	...
	return types.EventPtr(types.Event{Type: types.EvStepObserved, Payload: types.StepObserved{
