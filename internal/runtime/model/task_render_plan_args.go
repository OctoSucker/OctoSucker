...
func (t *Task) RenderPlanStepArguments(stepID string) (map[string]any, error) {
	if t == nil || t.Plan == nil {
		return nil, fmt.Errorf("task: render plan step arguments: nil task or plan")
	}
	st := t.Plan.FindStep(stepID)
	if st == nil {
		return nil, fmt.Errorf("task: render plan step arguments: step %q not found", stepID)
	}
	if len(st.Arguments) == 0 {
		return map[string]any{}, nil
	}
	ctx, err := t.planStepArgTemplateContext()
	if err != nil {
		return nil, err
	}
...
func (t *Task) planStepArgTemplateContext() (map[string]any, error) {
	out := map[string]any{}
	if s := strings.TrimSpace(t.UserInput); s != "" {
		out["user_input"] = s
	}
	if s := strings.TrimSpace(t.EvidenceSummary); s != "" {
		out["evidence"] = s
		out["evidence_summary"] = s
	}
	sum, err := StepSummariesFromPlan(t.Plan)
	if err != nil {
		return nil, err
	}
	...
	lastID, lastText, err := LastDoneStepPrimary(t.Plan)
	if err != nil {
		return nil, err
	}
	if lastText != "" {
