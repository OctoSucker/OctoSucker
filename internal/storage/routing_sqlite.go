...
// EdgeKey identifies a directed edge: canonical tool ids (Node.String() in tool routing), "" for entry → tool.
type EdgeKey struct {
	From string
	To   string
}
...
// RoutingEdgeRow is one routing_edges row and the in-memory per-edge stats shape used by toolrouting.Graph.
