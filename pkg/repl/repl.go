// Package repl provides a shared stdin REPL for cmd binaries (e.g. cmd/executor, cmd/thinker).
package repl

import (
	"bufio"
	"context"
	"errors"
	"fmt"
	"io"
	"os"
	"strings"
)

const (
	cmdInputPrompt = "🐙 > "
	// MsgAgentBusy is returned when another ingress holds the single-flight turn lock.
	MsgAgentBusy = "当前 agent 正忙，请稍后再试。"
)

type OnMessage func(ctx context.Context, text string) ([]string, error)

// RunCmdREPL reads lines from stdin and dispatches them through onMessage until ctx is done or EOF.
func RunCmdREPL(ctx context.Context, onMessage OnMessage, logPath string) error {
	fmt.Fprintf(os.Stdout, "cmd: 输入消息进行对话（详细日志: %s，可用 tail -f 查看）\n", logPath)
	rd := bufio.NewReader(os.Stdin)
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
		}
		fmt.Fprint(os.Stdout, cmdInputPrompt)
		line, err := rd.ReadString('\n')
		if err != nil {
			if errors.Is(err, io.EOF) {
				return nil
			}
			return err
		}
		line = strings.TrimSpace(line)
		if line == "" {
			continue
		}
		msgs, err := onMessage(ctx, line)
		if err != nil {
			fmt.Fprintf(os.Stdout, "error: %v\n", err)
			continue
		}
		for _, m := range msgs {
			fmt.Fprintln(os.Stdout, m)
		}
	}
}
